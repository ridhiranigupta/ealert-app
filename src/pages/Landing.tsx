...
function SectionTitle({
  ...
  return (
    ...
    >
      <p className="font-mono text-xs uppercase tracking-[0.25em] text-violet-300">{eyebrow}</p>
  ...
  return (
    <div className="relative min-h-screen overflow-x-clip bg-background">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        <div className="absolute right-[-120px] top-1/3 h-80 w-80 rounded-full bg-cyan-500/15 blur-3xl" />
      ...
      <section id="top" className="mx-auto max-w-6xl px-4 pb-20 pt-14 sm:px-6 sm:pt-20">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          ...
          >
            <p className="inline-flex items-center gap-2 rounded-full border border-violet-400/25 bg-violet-500/10 px-3.5 py-1.5 font-mono text-[11px] uppercase tracking-widest text-violet-200">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              ...
              <Button asChild size="lg" variant="outline" className="rounded-2xl border-white/15 bg-white/[0.03] hover:bg-white/[0.08]">
                <a href="#how">Learn more</a>
              </Button>
            </div>
            <div className="mt-10 flex items-center gap-8 border-t border-white/[0.08] pt-6">
              {[
                ["3s", "hold to alert"],
                ["10", "max contacts"],
                ["24/7", "always ready"],
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f, i) => (
            <motion.div
              ...
              className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.05]"
            >
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((s, i) => (
            ...
            >
              ...
              {i < steps.length - 1 && (
                <ArrowRight className="absolute -right-3.5 top-1/2 hidden size-5 -translate-y-1/2 text-white/20 lg:block" />
              )}
            </motion.div>
          ))}
        </div>
      ...
      <section id="safety" className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          ...
          >
            <p className="font-mono text-xs uppercase tracking-[0.25em] text-violet-300">Safety</p>
            ...
            <ul className="mt-7 space-y-4">
              {[
                ...
                <li key={t} className="flex gap-3.5">
                  <span className="mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full bg-violet-500/15 text-violet-300">
      ...
      <section id="about" className="mx-auto max-w-6xl px-4 pb-20 sm:px-6">
        ...
        >
          <div className="pointer-events-none absolute -top-20 left-1/2 h-48 w-[480px] -translate-x-1/2 rounded-full bg-violet-600/25 blur-3xl" />
      ...
      <footer className="border-t border-white/[0.07]">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
          ...
          <div className="mt-10 flex flex-col items-start justify-between gap-3 border-t border-white/[0.07] pt-6 sm:flex-row sm:items-center">
...
function AnimatedSafetyArt() {
  return (
    <div className="flex items-center justify-center gap-4">
      <span className="animate-float-y rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-center">
      ...
      <span className="animate-float-y rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-center" style={{ animationDelay: "0.8s" }}>
        <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">role</p>
        <p className="mt-1 text-sm font-semibold text-violet-300">admin · user</p>
      </span>
      <span className="animate-float-y rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-center" style={{ animationDelay: "1.6s" }}>
        <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">audit</p>
        <p className="mt-1 text-sm font-semibold text-cyan-300">logged</p>
      </span>
    </div>
  );
}
